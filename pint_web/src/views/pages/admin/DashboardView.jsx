...
import { getDashboardAdmin } from '../../../controllers/dashboard.controller'


const MONTH_LABELS = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez']
const LP_COLORS = ['#3A57E8', '#85F4FA']
const EMPTY_LEVEL = { label: '', chartSeries: [], paths: [] }
...
const learningPathChartOptions = {
  ...
  plotOptions: {
    radialBar: {
      ...
      track: { background: '#E9ECEF', strokeWidth: '100%', margin: 8 },
      dataLabels: { show: false },
    },
  },
  stroke: { lineCap: 'round' },
  labels: ['Jornada Técnica', 'Power Skills'],
}
...
const areaChartOptions = {
  ...
  xaxis: {
    ...
    labels: { style: { colors: '#232D42', fontSize: '13px', fontWeight: 400 } },
  },
  yaxis: {
    labels: { style: { colors: '#232D42', fontSize: '13px', fontWeight: 400 } },
  },
...
const badgesGroupedChartOptions = {
  ...
  grid: { borderColor: '#E9ECEF', strokeDashArray: 0, xaxis: { lines: { show: false } } },
  tooltip: {
    shared: true,
    intersect: false,
...
const iconSvgProps = { xmlns: 'http://www.w3.org/2000/svg', fill: 'none', 'aria-hidden': true }
...
function MetricIcon({ icon }) {
  if (icon === 'users') return <DashboardUsersIcon />
  if (icon === 'badges') return <DashboardBadgesIcon />
  if (icon === 'service-lines') return <DashboardServiceLinesIcon />
  if (icon === 'areas') return <DashboardAreasIcon />
  if (icon === 'learning-paths') return <DashboardLearningPathsIcon />
...
function DashboardView() {
  ...
  const metrics = useMemo(() => {
    if (!data) return []
    return [
      { label: 'Utilizadores', value: data.total_utilizadores, icon: 'users' },
      { label: 'Service Lines', value: data.total_service_lines, icon: 'service-lines' },
      { label: 'Áreas', value: data.total_areas, icon: 'areas' },
      { label: 'Learning Paths', value: data.total_learning_paths, icon: 'learning-paths' },
      { label: 'Badges', value: data.total_badges, icon: 'badges' },
    ]
  }, [data])
  ...
  const selectedLearningPathLevel = useMemo(
    () => learningPathBadgeLevels.find(l => l.value === selectedLevel) ?? learningPathBadgeLevels[0] ?? EMPTY_LEVEL,
  ...
  return (
    <section className="softinsa-dashboard-page">
      <div className="softinsa-dashboard-hero">
        <h1>Olá, {data.nome_administrador}!</h1>
      </div>
            ...
            <Card.Header className="softinsa-chart-card-header softinsa-learning-path-card-header">
              <div className="softinsa-learning-path-card-title-wrap">
                ...
                <Dropdown className="softinsa-learning-path-dropdown">
                  <Dropdown.Toggle variant="link" id="softinsa-learning-path-dropdown" className="softinsa-learning-path-dropdown-toggle">
              ...
              <div className="softinsa-learning-path-legend" aria-label="Quantidade por learning path">
                {selectedLearningPathLevel.paths.map((path) => (
                  <div key={path.label} className="softinsa-learning-path-legend-item">
                    <span className="softinsa-learning-path-legend-dot" style={{ backgroundColor: path.color }} aria-hidden="true" />
