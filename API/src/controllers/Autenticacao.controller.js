...
controllers.updateUser = async (req, res) => {
    try {
        const { nome, email, imagem, password } = req.body;
        ...
        if (nome) user.nome_utilizador = nome;
        if (email) user.email_utilizador = email;
        if (imagem) user.imagem_utilizador = imagem;
        if (password) user.password_utilizador = await bcrypt.hash(password, 10);
