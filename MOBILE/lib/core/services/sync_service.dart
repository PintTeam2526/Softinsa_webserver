...
class SyncService {
  ...
  Future<String> _saveImageLocally(String? base64String, String folder,
    ...
    try {
      final bytes = base64Decode(base64String.contains(',') ? base64String
          .split(',')
          .last : base64String);
  ...
  Future<void> _performSync(int idConsultor) async {
    print(
        ">>> [SYNC] Sincronização GLOBAL iniciada para Consultor $idConsultor...");
    try {
      if (idConsultor > 0) {
        await pushPendingData(
            'objetivos', '/objetivos/adicionar', 'ID_OBJETIVO',
            idConsultor: idConsultor);
        await pushPendingData(
            'documentacoes', '/candidaturas/documentacao', 'ID_DOCUMENTACAO',
            idConsultor: idConsultor);
        await pushPendingData(
            'pedidosBadge', '/candidaturas/candidatar', 'ID_PEDIDO_BADGE',
            idConsultor: idConsultor);
        await pushPendingData('badgesFavoritos', '/favorito/set', 'ID_BADGE',
            idConsultor: idConsultor);
      }
  ...
  Future<void> syncTableByName(String tableName,
      {int? idConsultor, bool force = false, bool ignoreLastUpdate = false}) async {
    ...
    try {
      ...
      switch (tableName) {
        case 'learningPaths':
          endpoint = '/syncMobile/learningpaths';
          break;
        case 'serviceLines':
          endpoint = '/syncMobile/servicelines';
          break;
        case 'areas':
          endpoint = '/syncMobile/areas';
          break;
        case 'badges':
          endpoint = '/syncMobile/badges';
          break;
        case 'requisitos':
          endpoint = '/syncMobile/requisitos';
          break;
        case 'estados':
          endpoint = '/syncMobile/estados';
          break;
        case 'conquistas':
          if (idConsultor != null) {
            endpoint = '/conquistas/mobile/get/$idConsultor';
        ...
        case 'consultores':
          if (idConsultor != null) {
            endpoint = '/consultores/info/$idConsultor';
            ignoreLastUpdate = true;
            isSingle = false;
          }
          break;
        case 'badgesConcluidos':
          if (idConsultor != null)
            endpoint = '/syncMobile/badgesConcluidos/$idConsultor';
          break;
        case 'objetivos':
          if (idConsultor != null)
            endpoint = '/syncMobile/objetivos/$idConsultor';
          break;
        case 'notificacoes':
          if (idConsultor != null)
            endpoint = '/syncMobile/notificacoes/$idConsultor';
          break;
        case 'pedidosBadge':
          if (idConsultor != null)
            endpoint = '/syncMobile/pedidosBadges/$idConsultor';
          break;
        case 'historicoPedidos':
          if (idConsultor != null)
            endpoint = '/syncMobile/historicoPedidos/$idConsultor';
          break;
        case 'documentacoes':
          if (idConsultor != null)
            endpoint = '/syncMobile/documentacoes/$idConsultor';
          break;
        case 'conquistasConsultores':
          if (idConsultor != null)
            endpoint = '/syncMobile/conquistasConsultores/$idConsultor';
          break;
        case 'badgesRecomendados':
          endpoint = '/syncMobile/badges/recomendados';
          idConsultor = null;
          ignoreLastUpdate = true;
          break;
        case 'badgesFavoritos' :
          endpoint = '/syncMobile/badges/favoritos';
          idConsultor = null;
          ignoreLastUpdate = true;
          break;
      }
  ...
  Future<void> _syncTable({
    ...
    try {
      ...
      if (!ignoreLastUpdate && lastUpdate != null && lastUpdate.isNotEmpty) {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        else if (decoded is Map && (decoded.containsKey('recomendados')))
        ...
        for (var item in remoteData) {
          ...
          try {
            Map<String, dynamic> row = await _mapModelToTableWithFiles(
                tableName, item, idConsultor);
            if (row.isNotEmpty) {
              if (idConsultor != null && _syncingUserId != idConsultor) return;
              final String serverDate = item['updatedAt'] ??
                  item['updated_at'] ??
            ...
            print("Erro mapeamento $tableName: $e");
          }
        }
      ...
      print("Erro rede/sync $tableName: $e");
    }
  }

  // Novo método para mapear modelos e converter Base64 em Ficheiros
  Future<Map<String, dynamic>> _mapModelToTableWithFiles(String tableName,
      dynamic item, int? idConsultor) async {
    try {
      switch (tableName) {
        case 'consultores':
          var m = ConsultoresModel.fromJson(item);
          String localPath = await _saveImageLocally(
              m.imagemPerfil, 'perfil', 'user_${m.id}');
          return m.copyWith(imagemPerfil: localPath).toMap();

        case 'learningPaths':
          final m = LearningPathsModel.fromJson(item);
          String localPath = await _saveImageLocally(
              m.imagem, 'lp', 'lp_${m.id}');
          return {
            'ID_LEARNINGPATH': m.id,
            'NOME_LEARNINGPATH': m.nome,
            'DESCRICAO_LEARNINGPATH': m.descricao,
            'IMAGEM_LEARNING_PATH': localPath,
            'ESTADO_A_I_': m.estado_a_i ? 1 : 0,
            'DATA_INSERCAO': m.data_insercao
          };

        case 'serviceLines':
          final m = ServiceLinesModel.fromJson(item);
          String localPath = await _saveImageLocally(
              m.imagem, 'sl', 'sl_${m.id}');
          return {
            'ID_SERVICELINE': m.id,
            'ID_LEARNINGPATH': m.id_learning_path,
            'NOME_SERVICELINE': m.nome,
            'DESCRICAO_SERVICELINE': m.descricao,
            'IMAGEM_SERVICE_LINE': localPath,
            'ESTADO_A_I_': m.estado_a_i ? 1 : 0,
            'DATA_INSERCAO': m.data_insercao,
            'NOME_LP_PAI': m.nome_learning_path_pai ?? ''
          };
        ...
        case 'areas':
          ...
          String localPath = await _saveImageLocally(
              m.imagem, 'areas', 'area_${m.id}');
          return {
            'id_area': m.id,
            'id_service_line': m.id_service_line,
            'nome_area': m.nome,
            'descricao_area': m.descricao,
            'imagem_area': localPath,
            'estado_a_i': m.estado_a_i ? 1 : 0,
            'data_insercao': m.data_insercao,
            'nome_service_line': m.nome_service_line_pai ?? ''
          };

        case 'badges':
          final m = BadgesModel.fromJson(item);
          String localPath = await _saveImageLocally(
              m.imagem, 'badges', 'badge_${m.id}');
          return {
            'ID_BADGE': m.id,
            'ID_AREA': m.id_area,
            'NOME_BADGE': m.nome,
            'DESCRICAO_BADGE': m.descricao,
            'PONTOS_BADGE': m.pontos,
            'PAGO': m.pago_S_N ? 1 : 0,
            'NIVEL_BADGE': m.nivel,
            'IMAGEM_BADGE': localPath,
            'nome_area_pai': m.nome_area_pai ?? '',
            'ESTADO_A_I_': m.estado_a_i ? 1 : 0,
            'DATA_INSERCAO': m.data_insercao
          };
        ...
        case 'requisitos':
          ...
          String localPath = await _saveImageLocally(
              m.imagem, 'requisitos', 'req_${m.id}');
          return {
            'ID_REQUISITO': m.id,
            'ID_BADGE': m.id_badge,
            'NOME_REQUISITO': m.nome,
            'DESCRICAO_REQUISITO': m.descricao,
            'IMAGEM_REQUISITO': localPath,
            'ESTADO_A_I_': m.estado_a_i ? 1 : 0
          };
        ...
        case 'badgesConcluidos':
          ...
          String localPath = await _saveImageLocally(
              m.imagemBadge, 'badges_concluidos', 'bc_${m.idBadgeConcluido}');
          return {
            'ID_BADGE_CONCLUIDO': m.idBadgeConcluido,
            'ID_CONSULTOR': idConsultor ?? m.idBadgeConcluido,
            'ID_BADGE': m.idBadge,
            'NOME_BADGE': m.nomeBadge,
            'nome_area_pai': m.nomeAreaPai,
            'NIVEL_BADGE': m.nivelBadge,
            'PONTOS_BADGE': m.pontosBadge,
            'IMAGEM_BADGE': localPath,
            'DATA_CONCLUSAO': m.dataConclusao,
            'VALIDADE': m.validadeDias,
            'nome_sl_pai': m.nomeServiceLine
          };
        ...
        case 'objetivos':
          ...
          return {
            'ID_OBJETIVO': m.id,
            'ID_BADGE': m.id_badge,
            'ID_CONSULTOR': idConsultor ?? m.id_consultor,
            'NOME_OBJETIVO': m.nome,
            'DATA_LIMITE_CONCLUSAO': m.data_limite_conclusao,
            'DATA_CONCLUSAO_OBJETIVO': m.data_conclusao_objetivo ?? ''
          };

        case 'notificacoes':
          final m = NotificacoesModel.fromJson(item);
          return {
            'ID_NOTIFICACAO': m.idNotificacao,
            'ID_CONSULTOR': m.idConsultor,
            'NOTIFICACAO': m.notificacao,
            'DATA_DE_ENVIO': m.data_de_envio,
        ...
        case 'estados':
          ...
          return {
            ...
            'NOME_ESTADO': m.nome,
            'DESCRICAO_ESTADO': m.descricao
          };

        case 'pedidosBadge':
          final m = PedidosBadgeModel.fromJson(item);
          return {
            'ID_PEDIDO_BADGE': m.idPedidoBadge,
            'ID_CONSULTOR': m.idConsultor,
            'ID_BADGE': m.idBadge,
            'ESTADO_ATUAL': m.idEstadoAtual
          };

        case 'historicoPedidos':
          final m = HistoricoPedidosModel.fromJson(item);
          return {
            'ID_HISTORICO': m.idHistorico,
            'ID_BADGE': m.idBadge,
            'ID_CONSULTOR': m.idConsultor,
            'DATA': m.data
          };
        ...
        case 'documentacoes':
          ...
          return {
            'ID_DOCUMENTACAO': m.id,
            'ID_HISTORICO': m.id_historico,
            'ID_CONSULTOR': m.id_consultor,
        ...
        case 'conquistasConsultores':
          ...
          return {
            'ID_CONQUISTA_CONSULTOR': m.id_conquista_consultor,
            'ID_CONSULTOR': m.id_consultor,
            'ID_CONQUISTA': m.id_conquista
          };

        case 'badgesRecomendados':
          final m = BadgesRecomendadosModel.fromJson(item);
          String localPath = await _saveImageLocally(
              m.imagemBadge, 'recomendados', 'rec_${m.idBadge}');
          return {
            'ID_BADGE': m.idBadge,
            'NOME_BADGE': m.nomeBadge,
            'IMAGEM_BADGE': localPath
  ...
  Future<void> pushPendingData(String tableName, String endpoint,
      String idColumn, {int? idConsultor}) async {
      ...
      for (var row in pending) {
        try {
          ...
          if (response.statusCode == 201 || response.statusCode == 200 ||
            ...
            await db.delete(
                tableName, where: '$idColumn = ?', whereArgs: [localId]);
            pushed = true;
          }
        } catch (e) {
          print("Erro push $tableName: $e");
        }
      }
    } finally {
      _syncingTables.remove(tableName);
      if (pushed && idConsultor != null) {
        await syncTableByName(tableName, idConsultor: idConsultor,
            force: true,
            ignoreLastUpdate: true);
      }
    }
  ...
  Map<String, dynamic> _mapRowToApi(String tableName,
    ...
    switch (tableName) {
      case 'objetivos':
        ...
        return {
          'idBadge': row['ID_BADGE'],
          'idConsultor': row['ID_CONSULTOR'],
          'nomeBadge': row['NOME_OBJETIVO'],
          'dataLimiteConclusao': isoDate
        };
      case 'pedidosBadge':
        return {
          'idBadge': row['ID_BADGE'],
  ...
  // ALTERAR PARA ISTO DEPOIS DOS TESTES LOCAIS
...
// }
  ...
  // VERSAO DO FORMAT DATE PARA API EM LOCALHOST (TESTES)
  String _formatDateForApi(String isoDateString) {
    try {
      // Converte para DateTime (trata formatos locais e ISO) e força UTC
      DateTime dt = DateTime.parse(isoDateString).toUtc();

      // Formata exatamente com 3 casas decimais nos milissegundos e o 'Z' no fim
      // Formato: YYYY-MM-DDTHH:mm:ss.SSS'Z'
      String formatted = DateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").format(dt);

      return formatted;
    } catch (e) {
      // Se der erro, NÃO deves enviar a string original para a API,
      // pois ela vai quebrar o banco de dados. Envia null ou uma data padrão.
      print('Erro ao formatar data: $e');
      return DateTime
          .now()
          .toUtc()
          .toIso8601String(); // Ou retorne outra string segura/nula
    }
  }
}